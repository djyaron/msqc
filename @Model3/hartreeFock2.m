function [orb,Eorb,Ehf,densitySaveOut] = hartreeFock2(H1,H2,S,Enuc,Nelec, ...
...
if ((size(densitySaveIn{ienv+1},1) == 0) && ...
      (size(densitySaveIn{1},1) == 0))
   Pn = density(ienv);
elseif (size(densitySaveIn{ienv+1},1) == 0)
   Pn = densitySaveIn{1};
else
   Pn = densitySaveIn{ienv+1};
end
